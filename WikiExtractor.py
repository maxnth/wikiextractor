...
class Extractor(object):
    ...
    def write_output(self, out, text):
        ...
        if options.write_raw_text:
            ...
            # Set first letter for directory name which includes the file
        ...
        if(not (options.write_raw_text or options.write_json)):
            if options.print_revision:
...
def main():
    ...
    groupO.add_argument("--raw_text", action="store_true",
                        help="write each article as raw text per file instead of the default one")
    ...
    options.write_raw_text = args.raw_text
